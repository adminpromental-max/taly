...
class PaymentController extends Controller
{
    ...
    public function findAmount($data, $orders = null, $active_currency = null)
    {
        ...
        if (arrayCheck('type', $data) && $data['type'] == 'wallet') {
            $amount = $data['amount'];
        } elseif (arrayCheck('saas_package_id', $data)) {
            $amount = SaasPackage::find($data['saas_package_id'])->price;
            if ($active_currency) {
    ...
    public function successUrl($request, $user = null, $amount = null): string|UrlGenerator|Application
    {
        $url   = '';
        $token = $request->token;
        ...
        } elseif ($request->saas_package_id) {
            $url = route('organization.saas.subscription', [
                'trx_id'          => $request->trx_id,
                'payment_type'    => $request->payment_type,
                'paymentID'       => $request->paymentID,
                'id'              => $request->saas_package_id,
                'saas_package_id' => $request->saas_package_id,
            ]);
    ...
    //pg redirects
    public function stripeRedirect(Request $request): Redirector|RedirectResponse|Application
    {
        try {
            $data            = $request->all();
    ...
    public function mollieRedirect(Request $request): Redirector|Application|RedirectResponse
    {
        try {
    ...
    {
        $data            = $request->all();
        if (! $this->getCurrency('NPR')) {
            Toastr::error('NPR currency is not available.');

            return back();
        }
        $currency        = $this->getCurrency('NPR');
        if (setting('is_esewa_sandbox_mode_activated')) {
        ...
        if (setting('esewa_merchant_id') && setting('esewa_secret_key')) {
            $signature = $this->generateESewaSignature($total_amount, setting('esewa_merchant_id'), setting('esewa_secret_key'));
        } else {
            Toastr::error('e-Sewa is not available at this moment.');
        ...
        $data            = [
            ...
            'transaction_uuid'        => $signature['transaction_uuid'],
            'product_code'            => setting('esewa_merchant_id'),
            'product_service_charge'  => 0,
            'product_delivery_charge' => 0,
            'success_url'             => $success_url,
            'failure_url'             => $this->removeParameterFromPrevious(),
            'signed_field_names'      => 'total_amount,transaction_uuid,product_code',
            'secret'                  => setting('esewa_secret_key'),
    ...
    public function sslRedirect(Request $request)
    {
        try {
            ...
            if (! $bdt_currency) {
                Toastr::error('BDT currency is not available.');
    ...
    public function bkashRedirect(Request $request): bool|Redirector|RedirectResponse|Application
    {
        try {
            $bdt_currency            = $this->getCurrency();
            if (! $bdt_currency) {
    ...
    public function nagadRedirect(Request $request): bool|Redirector|RedirectResponse|Application
    {
    ...
    public function nagadVerify(Request $request, OrderRepository $order): \Illuminate\Http\JsonResponse|Redirector|Application|RedirectResponse
    {
        $config   = [
    ...
    public function paytmRedirect(Request $request)
    {
        try {
            ...
            if (! $inr_currency) {
                Toastr::error('INR currency is not available.');
    ...
    public function razorpayRedirect(Request $request): \Illuminate\Http\JsonResponse
    {
        try {
            ...
            if (! $inr_currency) {
                return response()->json([
                    'error' => 'INR currency is not available.',
                ]);
            }
    ...
    public function fwRedirect(Request $request): RedirectResponse
    {
        try {
    ...
    public function mercadoPagoRedirect(Request $request): \Illuminate\Contracts\View\Factory|\Illuminate\Contracts\View\View|Application
    {
    ...
    public function midtransRedirect(Request $request): RedirectResponse
    {
    ...
    public function telrRedirect(Request $request): Redirector|Application|RedirectResponse
    {
        $data        = $request->all();
        $orders      = $this->findOrders($data);
    ...
    public function uddoktaPyRedirect(Request $request)
    {
        try {
            $bdt_currency    = $this->getCurrency();
            if (! $bdt_currency) {
                Toastr::error(__('bdt_currency_not_found'));
    ...
    public function tapRedirect(Request $request)
    {
        try {
            ...
            $user            = $this->findUser($data);
            $active_currency = $this->activeCurrencyCheck();
            if (! $this->getCurrency('KWD')) {
                Toastr::error('KWD currency is not available.');

                return back();
            }
            $amount          = $this->currencyAmountCalculator($orders, $data, $active_currency, $this->getCurrency('KWD'));
            $success_url     = $this->successUrl($request, $this->findUser($data), $amount['db_amount']);
            $tap_data        = [
                'amount'   => $amount['total_amount'],
                'currency' => 'KWD',
            ...
            if (! setting('tap_secret_key')) {
                Toastr::error('Tap is not available at this moment.');
            ...
            $headers         = [
                'Accept'        => 'application/json',
                'Content-Type'  => 'application/json',
                'Authorization' => 'Bearer '.setting('tap_secret_key'),
            ];
            $response        = httpRequest("$base_url/v2/charges", $tap_data, $headers);
