...
@section('content')
    @if($course->course_type == 'bundle')
        <section class="course-details-area p-b-50">
            ...
            <div class="container container-1278 my-4">
                <div class="row justify-content-center">
                    <div class="col-lg-6">
                        <div class="common-heading text-center m-b-40">
                            <h3>{{__('bundle_course')}}</h3>
                        </div>
                    </div>
                </div>
                <div class="course-items-wrap">
                    <div class="row course-items-v3 course-slider"
                        ...
                        @foreach($course->bundleCourse as $key => $bundleCourse)
                            @php
                                $bundle_course = $bundleCourse->course;
                            @endphp
                            <div class="col-md-12 col-sm-6">
                                <div class="course-item" data-aos="fade-up" data-aos-delay="{{ $key+=100 }}">
                                    <a href="javascript:void(0)" class="course-item-thumb">
                                        <img src="{{ getFileLink('295x248', $bundle_course->image) }}"
                                             alt="{{ $bundle_course->title }}">
                                    </a>
                                    <div class="course-item-body">
                                        <div class="course-item-header ">
                                            @if($bundle_course->category)
                                                <ul class="course-category ">
                                                    <li>
                                                        <a href="#">{{ $bundle_course->category->lang_title }}</a>
                                                    </li>
                                                </ul>
                                            @endif
                                            @if($bundle_course->total_rating)
                                                <div class="rating-review">
                                                    <span class="total-review"><i class="fas fa-star"></i>{{ number_format($bundle_course->total_rating,2) }}</span>
                                                </div>
                                            @endif
                                        </div>
                                        <h4 class="title">
                                            <a href="javascript:void(0)">{{ $bundle_course->title }}</a>
                                        </h4>
                                        <ul class="course-item-info {{ !$bundle_course->total_lesso && $bundle_course->total_enrolled == 0 ? 'mb-20' : '' }}">
                                            @if($bundle_course->total_lesson)
                                                <li>
                                                    <i class="fal fa-file-alt"></i> {{ $bundle_course->total_lesson }} {{__('lessons') }}
                                                </li>
                                            @endif
                                            @if($bundle_course->total_enrolled > 0)
                                                <li>
                                                    <i class="fal fa-user-friends"></i> {{ $bundle_course->total_enrolled }} {{__('enroll') }}
                                                </li>
                                            @endif
                                        </ul>
                                    </div>
                                ...
                                @if(auth()->check())
                                    @if( auth()->user()->user_type == 'student')
                                        <div
                                            ...
                                            <div
                                                class="course-enroll cart_area d-flex align-items-center justify-content-center gap-4">
                                                @if($is_enrolled)
                                                    <a href="{{ route('purchase.bundle', $course->slug) }}"
                                                       class="template-btn {{ auth() ?
                                                            'w-20' : 'w-20' }}">{{__('go_to_course') }}
                                                        <i class="fal fa-long-arrow-right"></i></a>
                                                @else
                                                    <a href="javascript:void(0)"
                                                       class="template-btn added_to_cart {{ auth() ?
                                                           'w-20' : 'w-20' }} {{ $is_added_to_cart ? '' : 'd-none' }}">{{__('added_to_cart')}}</a>
                                                    <a href="javascript:void(0)"
                                                       class="template-btn add_to_cart {{ auth() ?
                                                            'w-20' : 'w-20' }} {{ $is_added_to_cart ? 'd-none' : '' }}"
                                                ...
                                                <div class="col-md-9">
                                                    <div class="line-progress">
                                                        <p class="d-flex align-items-center justify-content-end mb-2">
                                                            <span>{{$hasEnrolled->complete_count}}% {{__('done')}}</span>
                                                        </p>
                                    ...
                                    <div class="comments-template comments-template-v2">
                                        @if($can_review)
                                            <div class="comments-respond" id="comment-respond">
                                                ...
                                                <div
                                                    class="rating-review rating_comment m-b-40 m-b-md-25 all-rating"></div>
