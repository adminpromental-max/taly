    ...
    public function boot()
    {
        ...
        view::composer(['frontend.layouts.base'], function ($view) {
            ...
            if (request()->routeIs('page.show')) {
                $page                     = \App\Models\Page::where('link', request()->route()->parameters()['link'])->first();
                $meta['meta_title']       = $page->meta_title ?: $page->title;
                $meta['meta_description'] = $page->meta_description ?: ($page->short_description ?: $page->description);
                $meta['meta_keywords']    = $page->meta_keywords ?: setting('meta_keywords').','.$page->title.','.$page->slug.','.$page->meta_title;
                $meta['meta_section']     = __('page');
                $meta['image_size']       = $page->meta_image ? 1200 : 400;
                $meta['meta_image']       = $page->meta_image ? getFileLink('1200x630', $page->meta_image) : getFileLink('406x240', $page->image);
            }
            $view->with('meta', $meta);
        });
    }
