...
class CourseController extends Controller
{
    ...
    public function edit($id, LanguageRepository $languageRepository, LevelRepository $levelRepository, TagRepository $tagRepository, SubjectRepository $subjectRepository, AssignmentDataTable $dataTable, Request $request)
    {
        try {
            ...
            //            try {
            //                $google_token       = json_decode(file_get_contents(config('google-calendar.auth_profiles.oauth.token_json')), true);
            //                $created_at         = Carbon::createFromTimestamp($google_token['created']);
            //                $expires_in         = $google_token['expires_in'];
            //                $expirationTime     = $created_at->copy()->addSeconds($expires_in);
            //                $google_token_check = $expirationTime->isPast();
            //            } catch (\Exception $e) {
            //                $google_token_check = false;
            //            }
            ...
            $data   = [
                ...
                //                'need_google_login' => $google_token_check,
    ...
    public function courseFeatureChange(Request $request): \Illuminate\Http\JsonResponse
    {
        if (config('app.demo_mode')) {
            $data = [
                'status'  => 'danger',
        ...
        try {
            $this->course->feature($request->all());
            $data = [
                'status'  => 'success',
                'message' => __('update_successful'),
                'title'   => 'success',
