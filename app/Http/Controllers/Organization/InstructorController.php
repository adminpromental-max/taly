...
class InstructorController extends Controller
{
    ...
    public function store(InstructorRequest $request, UserRepository $userRepository): \Illuminate\Http\JsonResponse
    {
        ...
        if (addon_is_activated('saas_subscription')) {
            $activePackage           = Subscription::where('org_id', authOrganizationId())->where('status', 1)->first();
            $existingInstructorCount = Instructor::where('organization_id', authOrganizationId())->count();
            if ($activePackage->total_instructor != -1 && $existingInstructorCount >= $activePackage->total_instructor) {
                $data = [
                    'status' => 'danger',
                    'error'  => __('insufficient_instructor_limit'),
                    'title'  => 'error',
                ];
