...
class AddonController extends Controller
{
    ...
    public function store(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        try {
            ...
            $addons      = File::allFiles(base_path('public/addons/'.$rand_str_dir.'/'));
            foreach ($addons as $addon) {
                $open_able             = $zip->open($addon->getRealPath());
                $file_name             = str_replace('.zip', '', $addon->getFilename());
                ...
                if ($open_able) {
                    $zip->extractTo(base_path("public/addons/$file_name/"));
                    $zip->close();
                } else {
                    return response()->json(['error' => 'Unable to open file, please try again']);
                }
                $get_addon_class       = glob(base_path('public/addons/'.$file_name.'/*Addon.php'));
                if (empty($get_addon_class)) {
                    File::deleteDirectory(base_path('public/addons/'.$file_name));
                ...
                try {
                    $read_json = file_get_contents(base_path("public/addons/$file_name/config.json"));
                } catch (\Exception $e) {
                    File::deleteDirectory(base_path('public/addons/'.$file_name));
                ...
                if ($addon_exist) {
                    File::deleteDirectory(base_path('app/Addons/'.$file_name));
                ...
                File::move(base_path('public/addons/'.$file_name), base_path('app/Addons/'.$file_name));
                Artisan::call('migrate', [
                    '--path'  => "app/Addons/$file_name/migrations",
                    '--force' => true,
                ]);
            }
            ...
            if (is_dir(base_path('app/addons/'.$rand_str_dir))) {
                File::deleteDirectory(base_path('app/Addons/'.$rand_str_dir));
            }
