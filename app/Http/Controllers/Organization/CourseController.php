    ...
    public function store(CourseRequest $request): \Illuminate\Http\RedirectResponse
    {
        try {
            ...
            if (addon_is_activated('saas_subscription')) {
                ...
                $existingCourseCount = Course::where('organization_id', authOrganizationId())->count();
                if ($activePackage->total_course != -1 && $existingCourseCount >= $activePackage->total_course) {
                    Toastr::error(__('insufficient_create_course_limit'));
