...
if (! function_exists('checkSubscription')) {
    function checkSubscription($check_type): string
    {
        $flag = true;
        if (addon_is_activated('saas_subscription')) {
            $user = auth()->user();
            if ($user->user_type == 'instructor') {
                $instructor   = $user->instructor;
                $subscription = Subscription::where('organization_id', $instructor->organization_id)->where('purchase_at', '<=', now())->where('expires_at', '>=', now())->first();
                if ($subscription) {
                    if ($check_type == 'course') {
                        $total_course = \App\Models\Course::where('organization_id', $instructor->organization_id)->count();
                        if ($total_course < $subscription->total_course) {
                            $flag = false;
                        }
                    }
                    if ($check_type == 'instructor') {
                        $total_course = \App\Models\Instructor::where('organization_id', $instructor->organization_id)->count();
                        if ($total_course < $subscription->total_instructor) {
                            $flag = false;
                        }
                    }
                } else {
