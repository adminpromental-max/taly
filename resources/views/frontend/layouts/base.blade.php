...
<head>
    ...
    <title>@yield('title') | {{ setting('system_name') != '' ? setting('system_name') : 'Taly' }}</title>
    ...
    @else
        <link rel="shortcut icon" href="{{ static_asset('frontend/img/taly/taly-icon.png') }}">
    @endif
    <!--====== Bootstrap CSS ======-->
...
@if (setting('disable_preloader') != '1')
    <div class="preloader">
        <div class="loading">
            <img
                src="{{ setting('preloader_logo') && @is_file_exists(setting('preloader_logo')['original_image']) ? get_media(setting('preloader_logo')['original_image']) : static_asset('frontend/img/taly/taly-icon.png') }}"
                alt="{{ setting('system_title') ?: 'Taly' }}">
        </div>
    </div>
@endif
@yield('base.content')
