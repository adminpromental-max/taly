...
class CourseRepository
{
    ...
    public function all()
    {
        return Course::orderByDesc('id')->where('course_type', '!=', 'bundle')->paginate(setting('paginate'));
    }
    ...
    public function store($request)
    {
        ...
        $request['user_id']     = auth()->id();
        $request['is_featured'] = arrayCheck('is_featured', $request) ? $request['is_featured'] : '0';
    ...
    {
        return Course::withAvg('reviews', 'rating')->withCount('reviews')->with('bundleCourse', 'users')->withCount('enrolls')->find($id);
    }

    public function CourseFindBySlug($slug , $is_my_course = false)
    {
    ...
    {
        return Course::with('wishlists', 'bundleCourse.Course')->where('slug', $slug)->when($is_my_course, function ($query) {
    ...
    {
        return Course::with($with)->withAvg('reviews', 'rating')->where('is_private', 0)->withCount('reviews')->whereIn('id', $ids)->get();
    }
    ...
    public function activeCourses($data, $relation = [])
    {
        return Course::with($relation)->withCount('lessons')->withCount('enrolls')->withAvg('reviews', 'rating')
            ...
            })->when(arrayCheck('except_bundle', $data), function ($query) {
                $query->where('course_type', '!=', 'bundle');
            })->when(arrayCheck('meeting', $data), function ($query) {
                $query->where('course_type', 'live_class');
            })->when(arrayCheck('bundle', $data), function ($query) {
                $query->where('course_type', 'bundle');
