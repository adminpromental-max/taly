...
class CourseController extends Controller
{
    ...
    public function purchaseBundle(Request $request): \Illuminate\Http\JsonResponse
    {
        try {
            ...
            return $this->responseWithSuccess('bundle_course_retrieved_successfully', $data);
        } catch (\Exception $e) {
            return $this->responseWithError($e->getMessage());
        }
    }

    public function bundleCourses(Request $request, $slug): \Illuminate\Http\JsonResponse
    {
        try {
            $bundle  = $this->courseRepository->findBySlug($slug);
            $courses = Course::whereIn('id', $bundle->bundleCourse->pluck('course_id')->toArray())->paginate(1);
            if ($courses->previousPageUrl()) {
                if ($courses->onLastPage()) {
                    $total_results = $courses->total();
                } else {
    ...
    public function myBundleCourse($slug, LessonRepository $lessonRepository, ResourceRepository $resourceRepository)
    {
        try {
            $course           = $this->courseRepository->CourseFindBySlug($slug, 'my_course');
            $courseIDs        = $this->courseRepository->activeCoursesIDs([
                'course_id'  => 1,
                'section_id' => 1,
                'user_id'    => auth()->id(),
                'paginate'   => setting('paginate'),
